...
import { DataType as DataType$1, Vector, BufferType as BufferType$1, Data as Data$1, Type as Type$1, Field, makeData, Float64, FixedSizeList as FixedSizeList$1, List as List$1 } from 'apache-arrow';
...
function forEachSegmentInPolygon(points, visitor, options = {}) {
    ...
    const isClosedEx = isClosed ||
        (equals$1(points[start], points[endPointIndex]) &&
            equals$1(points[start + 1], points[endPointIndex + 1]));
    if (!isClosedEx) {
...
function forEachSegmentInPolygonPoints(points, visitor, options = {}) {
    ...
    const isClosedEx = isClosed || equals$1(points[end - 1], points[0]);
    if (!isClosedEx) {
 ...
 */
function earcut$1(positions, holeIndices, dim = 2, areas, plane = 'xy') {
    const hasHoles = holeIndices.length;
    const outerLen = hasHoles ? holeIndices[0] * dim : positions.length;
    let outerNode = linkedList(positions, 0, outerLen, dim, true, areas, plane);
    const triangles = [];
...
function eliminateHoles(data, holeIndices, outerNode, dim, areas, plane) {
    ...
    for (i = 0, len = holeIndices.length; i < len; i++) {
        ...
        list = linkedList(data, start, end, dim, false, areas, plane);
        if (list === list.next)
            list.steiner = true;
...
function getMultiPointChild(input) {
    if ("data" in input) {
        return input.getChildAt(0);
    }
    return input.children[0];
}
function getMultiLineStringChild(input) {
    if ("data" in input) {
        return input.getChildAt(0);
    }
    return input.children[0];
}
function getMultiPolygonChild(input) {
    if ("data" in input) {
        return input.getChildAt(0);
    }
    return input.children[0];
...
var child = /*#__PURE__*/Object.freeze({
    __proto__: null,
    getLineStringChild: getLineStringChild,
    getMultiLineStringChild: getMultiLineStringChild,
    getMultiPointChild: getMultiPointChild,
    getMultiPolygonChild: getMultiPolygonChild,
    getPointChild: getPointChild,
    getPolygonChild: getPolygonChild
});

function makeMathGlPolygon(data, geomIndex) {
    ...
    const slicedFlatCoords = flatCoordData.values.subarray(coordsBegin * dim, coordsEnd * dim);
...
function signedArea(input) {
    if ("data" in input) {
        return new arrow.Vector(input.data.map((polygonData) => signedArea(polygonData)));
    }
    const result = new Float64Array(input.length);
    for (let geomIndex = 0; geomIndex < input.length; geomIndex++) {
        let polygon = makeMathGlPolygon(input, geomIndex);
...
function earcutSinglePolygon(data, geomIndex) {
    ...
    const triangles = earcut$1(slicedFlatCoords, holeIndices, dim);
...
function getPolygonExterior(input) {
    if ("data" in input) {
        return new arrow.Vector(input.data.map((data) => getPolygonExterior(data)));
    }
    return getPolygonChild(input);
}
function getMultiPolygonExterior(input) {
    if ("data" in input) {
        return new arrow.Vector(input.data.map((data) => getMultiPolygonExterior(data)));
    }
    return getMultiPolygonChild(input);
}

/** Check that the given type is a Point data type */
function isPoint(type) {
    if (DataType$1.isFixedSizeList(type)) {
        // Check list size
        if (![2, 3, 4].includes(type.listSize)) {
            return false;
        }
        // Check child of FixedSizeList is floating type
        if (!DataType$1.isFloat(type.children[0])) {
            return false;
        }
        return true;
    }
    if (DataType$1.isStruct(type)) {
        ...
        if (!type.children.every((field) => DataType$1.isFloat(field))) {
            return false;
        }
        return true;
    }
...
function isLineString(type) {
    // Check the outer type is a List
    if (!DataType$1.isList(type)) {
        return false;
    }
...
function isMultiLineString(type) {
    // Check the outer vector is a List
    if (!DataType$1.isList(type)) {
        return false;
    }
...
function isPointData(data) {
    return isPoint(data.type);
}
function isLineStringData(data) {
    return isLineString(data.type);
}
function isPolygonData(data) {
    return isPolygon(data.type);
}
function isMultiPointData(data) {
    return isMultiPoint(data.type);
}
function isMultiLineStringData(data) {
    return isMultiLineString(data.type);
}
function isMultiPolygonData(data) {
    return isMultiPolygon(data.type);
}

var data = /*#__PURE__*/Object.freeze({
    __proto__: null,
    isLineStringData: isLineStringData,
    isMultiLineStringData: isMultiLineStringData,
    isMultiPointData: isMultiPointData,
    isMultiPolygonData: isMultiPolygonData,
    isPointData: isPointData,
    isPolygonData: isPolygonData
});
...
function mapCoords(input, callback) {
...
function mapCoords0(input, callback) {
 ...
 */
function mapCoords1(input, callback) {
 ...
 * into effect for operating on coords
 */
function mapCoords2(input, callback) {
 ...
 * into effect for operating on coords
 */
function mapCoords3(input, callback) {
...
function sphere(a, b, rf, ellps, sphere) {
  if (!a) { // do we have an ellipsoid?
    var ellipse = match(exports, ellps);
    if (!ellipse) {
      ellipse = WGS84;
    }
    a = ellipse.a;
...
function reproject(input, fromProjection, toProjection) {
    const projectionFn = proj4(fromProjection, toProjection);
    // Check if an arrow.Vector
    if ("data" in input) {
        return new arrow.Vector(input.data.map((data) => reprojectData(data, projectionFn)));
    }
 ...
 */
function reprojectData(input, projectionFn) {
    // Avoid extra object creation
    const stack = [0, 0];
    const callback = (x, y) => {
        stack[0] = x;
        stack[1] = y;
        return projectionFn.forward(stack);
    };
...
(function (EXTENSION_NAME) {
    EXTENSION_NAME["POINT"] = "geoarrow.point";
    EXTENSION_NAME["LINESTRING"] = "geoarrow.linestring";
    EXTENSION_NAME["POLYGON"] = "geoarrow.polygon";
    EXTENSION_NAME["MULTIPOINT"] = "geoarrow.multipoint";
    EXTENSION_NAME["MULTILINESTRING"] = "geoarrow.multilinestring";
    EXTENSION_NAME["MULTIPOLYGON"] = "geoarrow.multipolygon";
...
function totalBounds(vector, field) {
...
function totalBoundsNest0(vector) {
    const bbox = new Bbox();
...
var Winding;
(function (Winding) {
    Winding[Winding["CLOCKWISE"] = WINDING.CLOCKWISE] = "CLOCKWISE";
    Winding[Winding["COUNTER_CLOCKWISE"] = WINDING.COUNTER_CLOCKWISE] = "COUNTER_CLOCKWISE";
})(Winding || (Winding = {}));
function windingDirection(input) {
    if ("data" in input) {
        return new arrow.Vector(input.data.map((polygonData) => windingDirection(polygonData)));
    }
...
function modifyWindingDirection(input, winding) {
    if ("data" in input) {
        input.data.forEach((polygonData) => modifyWindingDirection(polygonData, winding));
        return;
    }
    for (let geomIndex = 0; geomIndex < input.length; geomIndex++) {
...
var index$2 = /*#__PURE__*/Object.freeze({
    ...
    area: area,
    earcut: earcut,
    getMultiPolygonExterior: getMultiPolygonExterior,
    getPolygonExterior: getPolygonExterior,
    mapCoords: mapCoords,
    modifyWindingDirection: modifyWindingDirection,
    reproject: reproject,
...
function isPointVector(vector) {
    return isPoint(vector.type);
}
function isLineStringVector(vector) {
    return isLineString(vector.type);
}
function isPolygonVector(vector) {
    return isPolygon(vector.type);
}
function isMultiPointVector(vector) {
    return isMultiPoint(vector.type);
}
function isMultiLineStringVector(vector) {
    return isMultiLineString(vector.type);
}
function isMultiPolygonVector(vector) {
    return isMultiPolygon(vector.type);
}

var vector = /*#__PURE__*/Object.freeze({
    __proto__: null,
    isLineStringVector: isLineStringVector,
    isMultiLineStringVector: isMultiLineStringVector,
    isMultiPointVector: isMultiPointVector,
    isMultiPolygonVector: isMultiPolygonVector,
    isPointVector: isPointVector,
    isPolygonVector: isPolygonVector
});
...
function hardClone(data, force = false) {
    ...
    let clonedDictionary = undefined;
    ...
    const clonedBuffers = {
        [BufferType$1.OFFSET]: cloneBuffer(data.buffers[BufferType$1.OFFSET], force),
        [BufferType$1.DATA]: cloneBuffer(data.buffers[BufferType$1.DATA], force),
        [BufferType$1.VALIDITY]: cloneBuffer(data.buffers[BufferType$1.VALIDITY], force),
        [BufferType$1.TYPE]: cloneBuffer(data.buffers[BufferType$1.TYPE], force),
    };
    ...
    // sliced.
    return new Data$1(data.type, data.offset, data.length, 
...
function isShared(data) {
    ...
    const bufferTypes = [
        BufferType$1.OFFSET,
        BufferType$1.DATA,
        BufferType$1.VALIDITY,
        BufferType$1.TYPE,
    ];
    for (const bufferType of bufferTypes) {
        if (data.buffers[bufferType] !== undefined &&
...
function preparePostMessage(input, forceClone = false) {
    // Check if `input` is an arrow.Vector
    if ("data" in input) {
        const postMessageDatas = [];
        const transferArrayBuffers = [];
    ...
    if (input.dictionary !== undefined) {
        const [postMessageVector, arrayBuffers] = preparePostMessage(input.dictionary);
    ...
    if (input.buffers[BufferType$1.OFFSET] !== undefined) {
        // @ts-expect-error
        transferArrayBuffers.push(input.buffers[BufferType$1.OFFSET].buffer);
    }
    if (input.buffers[BufferType$1.DATA] !== undefined) {
        // @ts-expect-error
        transferArrayBuffers.push(input.buffers[BufferType$1.DATA].buffer);
    }
    if (input.buffers[BufferType$1.VALIDITY] !== undefined) {
        transferArrayBuffers.push(input.buffers[BufferType$1.VALIDITY].buffer);
    }
    if (input.buffers[BufferType$1.TYPE] !== undefined) {
        // @ts-expect-error
        transferArrayBuffers.push(input.buffers[BufferType$1.TYPE].buffer);
    }
...
function rehydrateType(type) {
    switch (type.typeId) {
        case Type$1.Null:
            return new Null();
        case Type$1.Int:
            // @ts-expect-error
            return new Int_(type.isSigned, type.bitWidth);
        case Type$1.Float:
            // @ts-expect-error
        ...
        case Type$1.Time:
            // @ts-expect-error
            return new Time_(type.unit, type.bitWidth);
        case Type$1.Timestamp:
            // @ts-expect-error
            return new Timestamp_(type.unit, type.timezone);
        case Type$1.Interval:
            // @ts-expect-error
            return new Interval_(type.unit);
        case Type$1.List: {
            const children = type.children.map(rehydrateField);
            if (children.length > 1)
                throw new Error("expected 1 field");
            // @ts-expect-error
        ...
        case Type$1.Union: {
            const children = type.children.map(rehydrateField);
            // @ts-expect-error
            return new Union_(type.mode, type.typeIds, children);
        }
...
function rehydrateData(data) {
    ...
    // attributes on data
    const buffers = {
        [BufferType$1.OFFSET]: data.valueOffsets,
        [BufferType$1.DATA]: data.values,
        [BufferType$1.VALIDITY]: data.nullBitmap,
        [BufferType$1.TYPE]: data.typeIds,
    };
    // @ts-expect-error
    return new Data(rehydrateType(data.type), data.offset, data.length, 
    // @ts-expect-error
    data._nullCount, buffers, children, dictionary);
}
/**
 ...
 * `preparePostMessage` to be accurately recreated.
 */
function rehydrateVector(vector) {
...
var index$1 = /*#__PURE__*/Object.freeze({
    ...
    rehydrateData: rehydrateData,
    rehydrateVector: rehydrateVector
});
...
var WKBType;
(function (WKBType) {
    WKBType[WKBType["Point"] = 0] = "Point";
    WKBType[WKBType["LineString"] = 1] = "LineString";
    WKBType[WKBType["Polygon"] = 2] = "Polygon";
    WKBType[WKBType["MultiPoint"] = 3] = "MultiPoint";
    WKBType[WKBType["MultiLineString"] = 4] = "MultiLineString";
    WKBType[WKBType["MultiPolygon"] = 5] = "MultiPolygon";
})(WKBType || (WKBType = {}));
/**
 ...
 */
function parseWkb(data, type, dim) {
    ...
    switch (type) {
        case WKBType.Point:
            return repackPoints(parsedGeometries, dim);
        case WKBType.LineString:
            return repackLineStrings(parsedGeometries, dim);
        case WKBType.Polygon:
            const repackedPolygons = repackPolygons(parsedGeometries, dim);
            console.log("here");
            console.log(repackedPolygons instanceof Data$1);
            return repackedPolygons;
        default:
            assertFalse("Not yet implemented for this geometry type");
    }
}
function* iterBinary(data) {
...
function repackPoints(geoms, dim) {
    ...
    return makeData({
        type: new FixedSizeList$1(dim, new Field(coordFieldName(dim), new Float64(), false)),
        child: coordsData,
    });
}
function repackLineStrings(geoms, dim) {
    ...
    const verticesData = makeData({
        type: new FixedSizeList$1(dim, new Field(coordFieldName(dim), coordsData.type, false)),
        child: coordsData,
    });
    return makeData({
        type: new List$1(new Field("vertices", verticesData.type, false)),
        valueOffsets: geomOffsets,
        child: verticesData,
    });
}
function inferLineStringCapacity(geoms) {
    let capacity = {
        coordCapacty: 0,
        geomCapacity: 0,
    };
...
function repackPolygons(geoms, dim) {
    ...
    for (const geom of geoms) {
        ...
        coords.set(geom.positions.value, coordOffset * dim);
        coordOffset += numCoords;
        for (let ringIdx = 0; ringIdx < geom.primitivePolygonIndices.value.length - 1; ringIdx++) {
    ...
    return makeData({
        type: new List$1(new Field("rings", ringsData.type, false)),
        valueOffsets: geomOffsets,
        child: ringsData,
    });
}
function inferPolygonCapacity(geoms) {
    let capacity = {
        coordCapacty: 0,
        ringCapacity: 0,
        geomCapacity: 0,
    ...
    for (const geom of geoms) {
        capacity.geomCapacity += 1;
        assert(geom.primitivePolygonIndices.value.length >= 1, "Expected primitivePolygonIndices to always have length at least 1");
        capacity.ringCapacity += geom.primitivePolygonIndices.value.length - 1;
        capacity.coordCapacty += geom.positions.value.length / geom.positions.size;
    }
    return capacity;
}
function coordFieldName(dim) {
    if (dim === 2) {
        return "xy";
    }
    else if (dim === 3) {
...
var index = /*#__PURE__*/Object.freeze({
    __proto__: null,
    get WKBType () { return WKBType; },
    parseWkb: parseWkb
});

export { index$2 as algorithm, child, data, index as io, type, vector, index$1 as worker };
//# sourceMappingURL=geoarrow.es.mjs.map
