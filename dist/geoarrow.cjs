...
function area(input) {
    if ("data" in input) {
        return new arrow__namespace.Vector(input.data.map((polygonData) => area(polygonData)));
    }
    ...
    return arrow__namespace.makeData({
        type: new arrow__namespace.Float(arrow__namespace.Precision.DOUBLE),
        length: input.length,
...
function signedArea(input) {
    if ("data" in input) {
        return new arrow__namespace.Vector(input.data.map((polygonData) => signedArea(polygonData)));
    }
    const result = new Float64Array(input.length);
    for (let geomIndex = 0; geomIndex < input.length; geomIndex++) {
        let polygon = makeMathGlPolygon(input, geomIndex);
...
function getPolygonExterior(input) {
    if ("data" in input) {
        return new arrow__namespace.Vector(input.data.map((data) => getPolygonExterior(data)));
    }
    return getPolygonChild(input);
}
function getMultiPolygonExterior(input) {
    if ("data" in input) {
        return new arrow__namespace.Vector(input.data.map((data) => getMultiPolygonExterior(data)));
    }
...
function mapCoords0(input, callback) {
    ...
    const newCoordsData = arrow__namespace.makeData({
...
function reproject(input, fromProjection, toProjection) {
    ...
    if ("data" in input) {
        return new arrow__namespace.Vector(input.data.map((data) => reprojectData(data, projectionFn)));
    }
...
function windingDirection(input) {
    if ("data" in input) {
        return new arrow__namespace.Vector(input.data.map((polygonData) => windingDirection(polygonData)));
    }
    let builder = new arrow__namespace.BoolBuilder({
        type: new arrow__namespace.Bool(),
        nullValues: [null],
    });
...
function parseWkb(data, type, dim) {
    ...
    switch (type) {
        ...
        case WKBType.Polygon:
            ...
            console.log("here");
            console.log(repackedPolygons instanceof arrow.Data);
...
function repackPoints(geoms, dim) {
    ...
    const coordsData = arrow.makeData({
        type: new arrow.Float64(),
        data: coords,
    });
    return arrow.makeData({
        type: new arrow.FixedSizeList(dim, new arrow.Field(coordFieldName(dim), new arrow.Float64(), false)),
        child: coordsData,
    });
}
function repackLineStrings(geoms, dim) {
    ...
    const verticesData = arrow.makeData({
        type: new arrow.FixedSizeList(dim, new arrow.Field(coordFieldName(dim), coordsData.type, false)),
        child: coordsData,
    });
    return arrow.makeData({
        type: new arrow.List(new arrow.Field("vertices", verticesData.type, false)),
...
function repackPolygons(geoms, dim) {
    ...
    const ringsData = arrow.makeData({
        type: new arrow.List(new arrow.Field("vertices", verticesData.type, false)),
        valueOffsets: ringOffsets,
        child: verticesData,
    });
    return arrow.makeData({
        type: new arrow.List(new arrow.Field("rings", ringsData.type, false)),
        valueOffsets: geomOffsets,
        child: ringsData,
    });
}
...
exports.algorithm = index$2;
exports.child = child;
exports.data = data;
exports.io = index;
exports.type = type;
exports.vector = vector;
exports.worker = index$1;
//# sourceMappingURL=geoarrow.cjs.map
